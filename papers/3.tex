\chapter{Culpability Without Ability: A Hard Determinist Proposal}
\chaptermark{Culpability Without Ability: A Hard Determinist Proposal}
\chapterauthor{Yuyang Ma,
...
is some earlier time $t$ and some course of action $A$ such that: 
\begin{enumerate}
\def\labelenumi{(\arabic{enumi})}
\setcounter{enumi}{0}
\item
  At $t$, $x$ was choosing between two or more incompatible courses of action, one
of which is $A$; 
\item 
  at $t$, $x$ ought to perform $A$ instead of what $x$ actually
did;
\item 
    if $x$ had performed $A$, $S$ would not have occurred;
\item 
    and at $t$, $x$ was able to perform $A$.
...
\noindent The Culpability Thesis interacts in a crucial way with the following
...
\noindent Both determinists and indeterminists can offer powerful arguments that
...
Argument.\footnote{Peter van Inwagen, ``Free Will and Moral
  ...
  Annual 38, no. 1 (2025): 13, \url{https://doi.org/10.5937/BPA2501007I}.} His
...
\section{The Moral Advice Argument}
...
\noindent For example, if it's Henry's fault that the cat died, then Henry should
...
do $A$.'\,''\footnote{van Inwagen, ``Free Will and Moral Responsibility,''
  15.}
\end{quote}

\noindent This step simply blocks the objection that the ``ought'' of advice might
...
$x$.''\footnote{van Inwagen, ``Free Will and Moral Responsibility,''
  16.}
\end{quote}

\noindent This is the critical step that establishes the `ought implies can'
...
\noindent To see how each steps play together, we can apply them to the Henry
...
\setcounter{enumi}{0}
\item
Suppose the cat's death is Henry's fault (starting premise).

\item 
By step one, Henry should have fed the cat.

\item 
By step two, if Henry deliberates whether to feed the cat or go to
Camp David, the correct answer to ``What should l do?'' is: You ought to
feed the cat.

\item 
By step three, for that advice to be correct, Henry must have been
able to feed the cat.

\item 
Therefore, if the cat's death was Henry's fault, Henry was able to
feed the cat.
\end{enumerate}

\subsection{Moral Advice as Attempt to Influence}
...
\vspace{-1em}
\begin{quote}
(CA.$\alpha$) In order for $X$ to give $Y$ the correct advice, $X$ must believe that
$Y$ is able to do the thing advised.
\end{quote}

\noindent Planner's advice, ``you ought to take Alison to Auberge du Vieux Puits
...
is able to do the thing advised, violating CA.$\alpha$.

If we build CA.$\alpha$ into step three, we get:
...
\noindent Following SS.3, van Inwagen's point is that anyone who rejects the
...
we can now replace CA.$\alpha$ with a more nuanced principle:

\vspace{-1em}
\begin{quote}
(CA.$\beta$) In order for $X$ to give $Y$ the correct advice, $X$ has to believe
that $Y$ is able to do the thing advised after receiving the advice.
\end{quote}

\noindent Going back to step three, notice that the strong version secretly builds
...
Under this conception of advice, a hard determinist can then build CA.$\beta$
...
\noindent We can see how the revised WS.3 can change the Moral Advice Argument by
...
\subsection{Moral Advice and Non-Cognitivism}
...
\subsection{Moral Advice and Hard Indeterminism}
...
To see why, it helps to state more precisely what CA.$\beta$ requires. On the
closest-world semantics for counterfactuals, CA.$\beta$ is satisfied when the
...
\noindent The outer counterfactual shifts us to the nearest possible world in
...
the case, then the relevant counterfactuals would be false, and CA.$\beta$
...
\subsection{Broad on `Ought Implies Can'}
...
\noindent Broad also holds that there are two general senses of `ought'. The first
...
\subsection{The Grammars of `Ought'}
...
\noindent For Broad, the grammatical form here is that we look back at what
...
\item
First, we still have the `comparative ought'. This Spinozan sense of
`ought', as sketched out by Broad, is certainly compatible with hard
determinism.

\item 
Second, we have the `deliberative ought'. This is the sense that
...
\item 
Third, we have the `retrospective ought'. This is the past-tense
...
\noindent The problem is how can hard determinists interpret the `retrospective
...
retrospective judgments of the form ``$X$ ought (not) to have done $\Phi$'' by
...
\begin{enumerate}[(R)]
\item 
    ``$X$ ought (not) to have done $\Phi$ at $t$'' $\Leftrightarrow$ ``If, at some suitable time
$t$*, prior to $t$, $X$ had asked `What should I do?', then the correct answer
would have been: `You ought (not) to do $\Phi$.'\,''
\end{enumerate}

\noindent For a hard determinist, to make a retrospective judgment and say ``you
...
have done $\Phi$'' are therefore identical to the truth-conditions of the
corresponding advisory claim ``You ought to do $\Phi$'' evaluated at an
...
in CA.$\beta$, without introducing any further commitment to categorical
ability.
...
\noindent On this view, fault-ascriptions, like advice, are part of the machinery
...
2014. 
\url{https://doi.org/10.4324/9781315823058}.
...
\url{https://doi.org/10.1017/9781316711101}.
...
7--19.
\url{https://doi.org/10.5937/BPA2501007I}.
